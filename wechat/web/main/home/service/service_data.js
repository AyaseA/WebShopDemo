$(function(){
        ...
        if (loadComplate) {
            if (pageNum * pageSize < allCount) {
                ...
                if ($(this).scrollTop() == maxScroll) {
                    ...
                    getProductServicesList(++pageNum, pageSize);
    ...
    // 加载商品列表
    function getProductServicesList(pn, ps) {
        var $proBox = $page.find('>div.main >div.content div.products');
        $$.get(
            'Product/StoreService/QueryProductServiceList?N=' + pn + '&Rows=' + ps,
            function(res) {
                if (res.Status != 0) {
                    return false;
                }
