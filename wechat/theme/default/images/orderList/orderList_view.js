...
//				$(".onePiece").append(piecePay);
				...
				if(listData.length==1){
					...
					$page.find(".all").append(onePiece);					
						...
						var onePiece="<div class='onePiece'><div class='pieceHeader'><span class='orderID'>"+list[i].ID+"</span><p class='pieceStatus'>"+showStatus(list[i].StatusID)+"</p></div><div class='pieceContent'><div></div><p>实付金额:<span>"+list[i].OutPocket+"</span></p></div><div class='piecePay'><button class='PayBtn'>支付</button></div></div>"
						$page.find(".all").append(onePiece);					
						}
