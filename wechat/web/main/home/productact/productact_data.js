 ...
 */
$(function() {
    var $page = $('#home_productact'),
        pageStr = 'home_productact',
    ...
    $page.off('click dbclick', '>div.footer >a.buyNow').on('click dbclick', '>div.footer >a.buyNow', function() {
        $$.redirect('home/fillOrder.html?pid=' + pid + '&num=' + pNum + '&type=5');
    });
    // 获取商品信息
    getProductInfo();
    // 修改立即购买按钮的商品id
    ...
    function getProductInfo() {
        $$.get(
            'Product/ProdMulti/QueryDetail?ID=' + pid,
            function(res) {
                if (res.Status != 0) {
                    return false;
                }
                if (res.Data) {
                    ...
                    if (d.ImgList) {
                        TouchSlide({
                            slideCell: "#home_productact_banner",
